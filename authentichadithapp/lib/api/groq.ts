...
export async function sendChatMessage(messages: ChatMessage[]): Promise<string> {
  ...
  for (const url of targetUrls) {
    // Bug fix: give each endpoint attempt (primary, fallback) its own
    // independent AbortController/timeout. A single shared controller meant
    // a slow primary poisoned the fallback's signal before it ever fired.
