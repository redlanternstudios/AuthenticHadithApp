...
describe("Splash & Brand Integrity Invariants (Anti-Black-Square Hardcode)", () => {
  // 1. splash-icon.png must be the authentic hadith emblem (>200KB, 1024x1024 RGBA)
  it("splash-icon.png exists, is > 200KB (not the 41KB Expo placeholder grid), and is 32-bit RGBA", () => {
    ...
    // Verify PNG magic bytes (0x89, 0x50)
  ...
  // 3. app.json must lock splash background to emerald (#1b5e43) across root, ios, and plugin configs
  it("app.json splash configuration is hardcoded to emerald (#1b5e43) for both light and dark mode", () => {
    ...
    expect(appJson.expo.splash.backgroundColor.toLowerCase()).toBe("#1b5e43");
    expect(appJson.expo.splash.dark.backgroundColor.toLowerCase()).toBe("#1b5e43");

    // iOS specific splash block
    expect(appJson.expo.ios.splash.image).toBe("./assets/images/splash-icon.png");
    expect(appJson.expo.ios.splash.backgroundColor.toLowerCase()).toBe("#1b5e43");
    expect(appJson.expo.ios.splash.dark.backgroundColor.toLowerCase()).toBe("#1b5e43");
    ...
    expect(config.backgroundColor.toLowerCase()).toBe("#1b5e43");
    ...
    expect(config.dark.backgroundColor.toLowerCase()).toBe("#1b5e43");
